...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        if (!(await Envs()))
            ...
            log(`\n============ 微信公众号：柠檬玩机交流 ============`)
            log(`\n=================== 共找到 ${mxbchdArr.length} 个账号 ===================`)
            if (debug) {
                log(`【debug】 这是你的全部账号数组:\n ${mxbchdArr}`);
            }
            for (let index = 0; index < mxbchdArr.length; index++) {

                let num = index + 1
                addNotifyStr(`\n==== 开始【第 ${num} 个账号】====\n`, true)

                mxbchd = mxbchdArr[index];            
...
async function checkin() {
 ...
 var options = {
  method: 'GET',
  url: 'https://mxsa.mxbc.net/api/v1/customer/signin?t='+timestamp+'&appId=d82be6bbc1da11eb9dd000163e122ecb&sign='+getSHA256withRSA('appId=d82be6bbc1da11eb9dd000163e122ecb&t='+timestamp),
  headers: {
'Host': 'mxsa.mxbc.net',
...
async function info() {
 ...
 var options = {
  method: 'GET',
  url: 'https://mxsa.mxbc.net/api/v1/customer/info?t='+timestamp+'&appId=d82be6bbc1da11eb9dd000163e122ecb&sign='+getSHA256withRSA('appId=d82be6bbc1da11eb9dd000163e122ecb&t='+timestamp),
  headers: {
'Host': 'mxsa.mxbc.net',
'Connection': 'keep-alive',
'user-agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.138 Safari/537.36 MicroMessenger/7.0.4.501 NetType/WIFI MiniProgramEnv/Windows WindowsWechat/WMPF',
...
async function Envs() {
    if (mxbchd) {
        if (mxbchd.indexOf("@") != -1) {
            mxbchd.split("@").forEach((item) => {

                mxbchdArr.push(item);
            });
        } else if (mxbchd.indexOf("\n") != -1) {
            mxbchd.split("\n").forEach((item) => {
                mxbchdArr.push(item);
            });
        } else {
            mxbchdArr.push(mxbchd);
        }
    } else {
        log(`\n 【${$.name}】：未填写变量 mxbchd`)
        return;
